...
MP4SampleId MP4Track::GetNextSyncSampleIdFromTime(
        MP4Timestamp when)
{
    uint32_t numStts = m_pSttsCountProperty->GetValue();
    MP4SampleId sid = 1;
...
MP4SampleId MP4Track::GetPreviousSyncSampleIdFromTime(
        MP4Timestamp when)
{
    uint32_t numStts = m_pSttsCountProperty->GetValue();
    MP4SampleId sid = 1;
    ...
    for (uint32_t sttsIndex = 0; sttsIndex < numStts; sttsIndex++) {
        ...
        if (d <= sampleCount * sampleDelta) {
            ...
            return GetPreviousSyncSample(sampleId);
        }
...
void MP4Track::GetSyncSampleIds(
        MP4SampleId** ppIds,uint32_t* pSize){
    *ppIds = NULL;
    *pSize = 0;
    ...
    if (*ppIds != NULL && *pSize < numStss) {
        throw new Exception("ids buffer is too small",
                            __FILE__, __LINE__, __FUNCTION__ );
    }
    *pSize = numStss;

    log.verbose3f("\"%s\": GetSyncSampleIds: track %u size %u (0x%x)",
                  GetFile().GetFilename().c_str(), m_trackId,  *pSize, *pSize);

    if (*ppIds == NULL) {
        *ppIds = (MP4SampleId*)MP4Malloc((*pSize) * sizeof(MP4SampleId));
    }

    for (uint32_t stssIndex = 0; stssIndex < numStss; stssIndex++) {
        (*ppIds)[stssIndex]= m_pStssSampleProperty->GetValue(stssIndex);
    }
}
...
// N.B. "previous" is inclusive of this sample id
MP4SampleId MP4Track::GetPreviousSyncSample(MP4SampleId sampleId)
{
    if (m_pStssCountProperty == NULL) {
        return sampleId;
    }
