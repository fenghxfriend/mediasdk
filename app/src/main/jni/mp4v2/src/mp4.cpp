...
// returns true if the track is a media track encrypted according to ismacryp
    ...
    MP4SampleId MP4GetNextSyncSampleIdFromTime(
            MP4FileHandle hFile,
            MP4TrackId trackId,
            MP4Timestamp when)
    {
        if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
            try {
                return ((MP4File*)hFile)->GetNextSyncSampleIdFromTime(
                        trackId, when);
            }
            catch( Exception* x ) {
                mp4v2::impl::log.errorf(*x);
...
MP4SampleId MP4GetPreviousSyncSampleIdFromTime(
        MP4FileHandle hFile,
        MP4TrackId trackId,
        MP4Timestamp when)
{
    if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
        try {
            return ((MP4File*)hFile)->GetPreviousSyncSampleIdFromTime(
                    trackId, when);
        }
        catch( Exception* x ) {
            mp4v2::impl::log.errorf(*x);
...
void MP4GetSyncSampleIds(
    ...
    if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
        try {
            ((MP4File*)hFile)->GetSyncSampleIds(
                    trackId, ppIds, pSize);
        }
        catch( Exception* x ) {
            mp4v2::impl::log.errorf(*x);
