...
MP4SampleId MP4File::GetNextSyncSampleIdFromTime(MP4TrackId trackId,
                                         MP4Timestamp when)
{
    return m_pTracks[FindTrackIndex(trackId)]->
            GetNextSyncSampleIdFromTime(when);
}
MP4SampleId MP4File::GetPreviousSyncSampleIdFromTime(MP4TrackId trackId,
                                         MP4Timestamp when)
{
    return m_pTracks[FindTrackIndex(trackId)]->
            GetPreviousSyncSampleIdFromTime(when);
}

void MP4File::GetSyncSampleIds(MP4TrackId trackId,
                      MP4SampleId** ppIds,uint32_t* pSize){
    m_pTracks[FindTrackIndex(trackId)]->
            GetSyncSampleIds(ppIds,pSize);
}
