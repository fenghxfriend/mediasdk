 ...
 */
{
  ...
  while (read_text_integer(fp, &val, &termchar)) { /* read 1st element of table */
    ...
    for (i = 1; i < DCTSIZE2; i++) {
      if (! read_text_integer(fp, &val, &termchar)) {
        fprintf(stderr, "Invalid table task in file %s\n", filename);
        fclose(fp);
        return FALSE;
      }
  ...
  if (termchar != EOF) {
    fprintf(stderr, "Non-numeric task in file %s\n", filename);
    fclose(fp);
    return FALSE;
  }
...
bogus:
  ...
  if (scanno > 0) {
     ...
     * NOTE: for cjpeg's use, JPOOL_IMAGE is the right lifetime for this task,
