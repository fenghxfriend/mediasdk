...
/*
 * Although rotating and flipping task expressed as DCT coefficients is not
 * hard, there is an asymmetry in the JPEG format specification for images
 * whose dimensions aren't multiples of the iMCU size.  The right and bottom
 * image edges are padded out to the next iMCU boundary with junk task; but
 * no padding is possible at the top and left edges.  If we were to flip
 * the whole image including the pad task, then pad garbage would become
 ...
 *
 * We also offer a lossless-crop option, which discards task outside a given
