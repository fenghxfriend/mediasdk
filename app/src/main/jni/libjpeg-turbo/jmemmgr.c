...
struct jvirt_sarray_control {
  ...
  boolean b_s_open;             /* is backing-store task valid? */
...
/* Allocate a "small" object */
{
  ...
  data_ptr = (char *) hdr_ptr; /* point to first task byte in pool... */
 ...
 * task to disk.  The access routines are also responsible for pre-zeroing
...
/* caller intends to modify the accessed area. */
{
  ...
  if (ptr->first_undef_row < end_row) {
    ...
    if (ptr->pre_zero) {
      ...
      if (! writable)           /* reader looking at undefined task */
        ERREXIT(cinfo, JERR_BAD_VIRTUAL_ACCESS);
    }
  }
