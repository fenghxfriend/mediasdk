...
code.  Some of the routines were incorrectly reading and storing task below the
...
if there were > 128 bytes of task in the input buffer.  However, it is possible
to construct a JPEG image in which a single Huffman block is over 430 bytes
long, so this version of libjpeg-turbo activates the accelerated Huffman
decoder only if there are > 512 bytes of task in the input buffer.
...
extremely-high-frequency block (basically junk image task) is being encoded.
...
high-frequency YUV task into the compressor), it was reproducible only once in
...
markers found in the middle of the JPEG task stream during decompression.  It
...
2. 2968313: Fixed task corruption issues when decompressing large JPEG images
