...
/*
 * To support 12-bit JPEG task, we'd have to scale output down to 8 bits.
 ...
 * from JPEG's top-to-bottom order.  To do this, we save the outgoing task
...
/* This version is for writing 24-bit pixels */
{
  ...
  /* Transfer task.  Note destination values must be in BGR order
...
#define PUT_4B(array,offset,value)  \
  ...
  /* we leave biSizeImage = 0; this is correct for uncompressed task */
                ...
                int map_colors, int map_entry_size)
{
  ...
  if (colormap != NULL) {
    ...
    /* If no colormap, must be grayscale task.  Generate a linear "map". */
    for (i = 0; i < 256; i++) {
      putc(i, outfile);
      putc(i, outfile);
      putc(i, outfile);
