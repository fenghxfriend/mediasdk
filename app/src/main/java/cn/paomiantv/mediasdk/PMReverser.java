...
public class PMReverser extends PMModule {
    public static final String TAG = PMReverser.class.getSimpleName();
    ...
    public PMReverser(String outputFile) {
        _create();
        this.mPMMuxer = new PMMuxer(outputFile);
    }

    private boolean onStartVEncode(String mime, int width, int height, int bitRate, int framerate, int iInterval, int profile, int level) {
        mvEncoderThread = new Thread(mvEncodeRunnable, "VReverseEncode");
    ...
    private boolean onStartAEncode(String mime, int sampleRate, int channels, int bitRate, int profile) {
        maEncoderThread = new Thread(maEncodeRunnable, "AReverseEncode");
...
//                    Log.e(TAG, "draw the frame: " + ts);
            ...
            break;
            case PMMessage.MESSAGE_ID_A_PLAY_PROGRESS: {
                try {
                    JSONObject json = new JSONObject(message);
                    long ts = json.optLong("timestamp", 0);
//                    Log.e(TAG, "play the frame: " + ts);
            ...
            break;
            case PMMessage.MESSAGE_ID_V_CUT_RENDER_COMPLETE: {
                Log.e(TAG, "video cut complete!!!!!!!!!!!!!!");
                vComplete = true;
                if (aComplete) {
            ...
            break;
            case PMMessage.MESSAGE_ID_A_CUT_PLAY_COMPLETE: {
                Log.e(TAG, "audio cut complete!!!!!!!!!!!!!!");
                aComplete = true;
                if (vComplete) {
                    Message msg = mHandler.obtainMessage();
                    msg.what = SUCCESS;
...
//            Log.e(TAG, "play the frame: " + timestampUS);
            ...
            ByteBuffer inpuyBuffer = maEncoder.getInputBuffers()[index];
            inpuyBuffer.clear();
            inpuyBuffer.position(0);
            inpuyBuffer.put(buffer, 0, buffer.length);
            maEncoder.queueInputBuffer(index, 0, inpuyBuffer.position(), timestampUS, 0);
        }
    }
    ...
    public void reverse(String path, boolean isVideo, boolean isAudio, String description) {
        this.mDescription = description;
        _reverse(PMEngine.getInstance().getSdk_temp_path(), path, isVideo, isAudio);
    }
    ...
    public void cancel() {
        ...
        if (mHandlerNativeMessage != null && mHandlerNativeMessage.get() != null) {
            Message msg = mHandlerNativeMessage.get().obtainMessage();
            msg.what = PMMessage.MESSAGE_ID_REVERSE_CANCELED;
    ...
    private Runnable mvEncodeRunnable = new Runnable() {
...
//                        Log.e(TAG, "encoder presentation time: " + info.presentationTimeUs);
    ...
    private Runnable maEncodeRunnable = new Runnable() {
        @Override
        public void run() {
            while (!isStop) {
                MediaCodec.BufferInfo info = new MediaCodec.BufferInfo();
...
//                        Log.d(TAG, "INFO_TRY_AGAIN_LATER");
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            if (mPMMuxer != null) {
                                ...
                                if (mHandlerNativeMessage != null && mHandlerNativeMessage.get() != null) {
                                    Message msg = mHandlerNativeMessage.get().obtainMessage();
                                    msg.what = PMMessage.MESSAGE_ID_REVERSE_COMPLETE;
                                    mHandlerNativeMessage.get().sendMessage(msg);
                                }
                            }
                            release();
    ...
    private native void _reverse(String tmp_path, String src, boolean isVideo, boolean isAudio);
    ...
    private static final int SUCCESS = PMMessage.MESSAGE_ID_REVERSE_COMPLETE + 2000;
    private static final int FAILED = PMMessage.MESSAGE_ID_REVERSE_CANCELED + 2000;
